...
type ReqClearBreakpoint struct {
	DebugToken string `json:"debugToken"`
	Line       int    `json:"line"`
}

func ApiClearBreakpoint(_ *gin.Context, req ReqSetBreakpoint) (code int, message string, data CommonResp) {
	resp := CommonResp{}
	if req.DebugToken == "" {
		// 没有传 token，直接退出
	...
	if err == nil {
		...
		return global.ERR_CLEAR_BREAKPOINT, fmt.Sprintf(global.ERR_CLEAR_BREAKPOINT_MESSAGE, err), resp
	}
}
